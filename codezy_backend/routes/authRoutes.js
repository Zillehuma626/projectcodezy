...
router.post("/login", async (req, res) => {
  try {
    ...
    // ========================
    // Events / Notifications (Notification already sent above)
    // ========================
    ...
    // ========================
    // Get student's class IDs if applicable
    // ========================
    let classIds = [];
    if (detectedRole === "student") {
      const studentRecord = await Student.findOne({ email: searchEmail }).select("classId");
      if (studentRecord?.classId) {
        classIds = [studentRecord.classId.toString()];
      }
    }
...
router.post("/verify-2fa", async (req, res) => {
  try {
    ...
    let classIds = [];
    if (role === "student") {
      const studentRecord = await Student.findById(user._id).select("classId");
      if (studentRecord?.classId) {
        classIds = [studentRecord.classId.toString()];
      }
    }
